...
3. 根据需要调整设置、打开本机预览或进入屏保。修改运行中的视频参数会断流，需重新连接。

默认：后置主摄、1080p、固定 30 FPS、H.264、4 Mbps。仅提供视频，不含音频、录像或公网穿透。

长时间使用建议插电并解除本应用的电池优化限制；重启或强制停止后需手动启动。RTSP 视频未加密，远程访问请使用可信 VPN。屏保可降低烧屏风险，不能保证永久零烧屏。
...
最低支持 Android 8；已在 Nothing A142 / Android 16 调试。当前通过 37 项 JVM 测试、13 项真机测试（含 128 组 GPU 比例校准），以及 H.264 / HEVC 的 TCP / UDP 播放回归。连续 24 小时稳定性尚未通过验证。
